...
class PickleIter(DataIter):
    """PickleIter object in mxnet. Taking path list of files which store data and label using pickle to get data iter.
    Parameters
    ----------
    datapath: list of str 
        list of file paths where data and label are stored in a dictionary
    batch_size: int
        Batch Size
    shuffle: bool
    ...
    def getdatafromnewfile(self):
        """Load data from underlying arrays, internal use only"""
        self.filecursor += 1
        assert(self.filecursor < self.num_datafile), "DataIter needs reset."
        with open(self.datapath[self.filecursor]) as f:
            d = cPickle.load(f) 
    ...
    def __init__(self, datapath, batch_size=1, shuffle=False, last_batch_handle='pad'):
        # pylint: disable=W0201
        ...
        self.filecursor = -1
        self.batch_size = batch_size
        self.datapath = filter(None,datapath)
        self.num_datafile = len(self.datapath) 
        self.shuffle = shuffle
        self.last_batch_handle = last_batch_handle
        self.getdatafromnewfile()
    ...
    def iter_next(self):
        ...
        if self.cursor < self.num_data:
            return 1 # next batch
        elif self.filecursor < self.num_datafile-1:
